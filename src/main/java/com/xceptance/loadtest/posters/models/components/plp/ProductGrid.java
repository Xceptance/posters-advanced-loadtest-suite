...
public class ProductGrid implements Component
{
    ...
    public List<String> getFilteredProductUrls()
    {
        ...
        List<String> discardedUrls = Context.configuration().filterProductUrls.unweightedList().stream()
                .filter(s -> StringUtils.isNotBlank(s)).collect(Collectors.toList());

        // Use either the embedded product state (if available) or existing product
        // tiles
    ...
    private List<String> getProductUrlsFromPageStateAndApplyFilter(List<String> discardedUrls)
    {
        ...
        for (int i = 0; i < products.length(); i++)
        {
            ...
            // Make sure id and name are contained in the date because we need it to create
            // the URL
            if (StringUtils.isBlank(id) || StringUtils.isBlank(name))
            {
                EventLogger.BROWSE.error("Invalid product URL details found at page",
                        Context.getPage().getUrl().toString());
                continue;
            }
            ...
            {
                EventLogger.BROWSE.error("Failed to encode URL created from product details found at page",
                        Context.getPage().getUrl().toString());
                continue;
            }
        }
        ...
        return productLinks.stream().filter(url -> !discardedUrls.stream().anyMatch(s -> url.contains(s)))
    ...
    private List<String> getProductUrlsFromExistingTilesAndApplyFilter(List<String> discardedUrls)
    {
        ...
        List<HtmlElement> productElements = locate()
                .byCss("div.product-display-case > div.product-tile > div.card-body > a[href]").all();

        // Retrieve all URLs and apply the URL filter
        return productElements.stream().map(e -> e.getAttribute("href"))
                .filter(url -> !discardedUrls.stream().anyMatch(filterUrl -> url.contains(filterUrl)))
                .collect(Collectors.toList());
    }
}